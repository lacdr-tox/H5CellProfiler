...
if( !all(unique(myDFrawImage$plateID) %in% unique(metaCSVData$plateID)) & 
      !all(unique(metaCSVData$plateID) %in% unique(myDFrawImage$plateID)))
{
  stop("Could not match all plateID's between manualy defined ID's and plate layout ID's")
}
...
for (childLoop in seq_along(child.objects))
{
  ...
  objName <- paste(objName, 'rowInd', sep = '_')
  currChildList <- lapply(currChildList, function(x) x<- setkeyv(x, objName))
...
if(any(imageInd))
  ...
  if(length(myDFImage$imageNumber) != length(unique(myDFImage$imageNumber)) |
       length(myDFImage$imageNumber) != nrow(myDFImage)){
    stop("duplicate image numbers in myDFImage")
  }
...
myDTParent[, eval(imName):=as.factor(get(imName))]
...
myDT[, dose_uM:=as.factor(dose_uM) ]
