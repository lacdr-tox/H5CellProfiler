...
rm(list=ls())
source("D:/R_HOME/R_WORK/R scipts/CPHDF5Analysis/mainFunction.R")
source("D:/R_HOME/R_WORK/R scipts/CPHDF5Analysis/fixTrackingFun.R")
library(rhdf5)
library(stringr)
library(plyr)
library(data.table)
...
source("D:/R_HOME/R_WORK/R scipts/CPHDF5Analysis/countCellFun.R")
...
hdf5FileNameL <- c("demo.h5") # mainFunction will loop through this vector, at the end rbinding the individual outputs. 
...
plateID <- c( "plate1") # PlateID must always be provided (either manual or h5 paths)- here AND in layout file: becuase plateID is used to couple metadata plate layout file.
imageID <- c("Image/Metadata_imageNr") # Image/Metadata_  ... image number (obtained from image file name ) 
...
exposureDelay <- c("00:20") # hh:mm 
timeBetweenFrames <- c("00:21:00") # hh:mm:ss 
...
firstFeature  = "cells_yfp/Intensity_MeanIntensity_YFP_over_CFP",  # enter a objectpath inside .h5 file to measurement ( there will be one  /  meaning--> object/measurement)
secondFeature = "Image/Count_Cells_mCherry",
thirdFeature  = "cells_yfp/AreaShape_Area",
fourthFeature = "cells_yfp/TrackObjects_DistanceTraveled_50",
fifthFeature  = "",
sixthFeature  = "",
seventhFeature= "",
eighthFeature = "",
...
plateMDFileName <- "2014_plate_layout.txt"
dir()
parentObject <- "cells_yfp" # the name in hdf5 file of the parent ( as defined in relate objects module in CellProfiler ) if no parent (only 1 object defined in CP for example: then enter the object here)
...
if(length(hdf5FileNameL) > 1) {
   cl<- makeCluster(min(numberCores, length(hdf5FileNameL)))
   registerDoSNOW(cl)
...
runApp('D:/R_HOME/R_WORK/R scipts/CPHDF5Analysis/time plots', launch.browser = TRUE )
...
if(makeQuantilePlots){
  # increase speed by using parallel processing and aggregate instead of ddply 
    if( objetSize > 800000000 & objetSize %/% 800000000 < 2 & length(whichQuantiles) > 4 ) {
      ...
      cl<-makeCluster(length(whichQuantiles))
      }
  registerDoSNOW(cl)
...
cl<-makeCluster(min(c(length(singleCellMeas[ singleCellMeas != "imageCountTracked"]), numberCores)))
      registerDoSNOW(cl)
...
#   myData_ss_c <- myData_ss_c[ !myData_ss_c$treatment %in%  c("Menadione", "etoposide " ,"Doxorubicin "), ]
 ...
 p<- ggplot( data = myData_ss_c,  aes( x = timeAfterExposure , y = meanSummaryStat, colour = variable)) + 
  ...
  cl<-makeCluster(min(c(length(allVarCells), numberCores)))
  registerDoSNOW(cl)
...
cl<-makeCluster(min(c(length(qVarsCells), numberCores)))
  registerDoSNOW(cl)
...
allTrackDF<-foreach(i = seq_along(uniqueWellGroups), 
  ...
  fixTrackingFun(myDFstukkie, myFeatures)
}
